    ...
    try:
        ...
        for edit in edits:
            ...
            if edit_type == "text":
                ...
                subtype = edit.get("subtype", "rect")
                x1 = edit.get("x1", 0); y1_raw = edit.get("y1", 0)
                x2 = edit.get("x2", 0); y2_raw = edit.get("y2", 0)
                # Convert from PDF bottom-left to PyMuPDF top-left
                my1 = page_height - y1_raw
                my2 = page_height - y2_raw
                color_hex = edit.get("color", "#ff0000")
                # Parse hex color
                color_hex = color_hex.lstrip('#')
                dr = int(color_hex[0:2],16)/255; dg = int(color_hex[2:4],16)/255; db = int(color_hex[4:6],16)/255
                draw_color = (dr, dg, db)
                width = edit.get("width", 2)
                if subtype == "rect":
                    page.draw_rect(fitz.Rect(min(x1,x2), min(my1,my2), max(x1,x2), max(my1,my2)),
                                   color=draw_color, width=width, overlay=True)
                elif subtype == "circle":
                    cx = (x1+x2)/2; cy = (my1+my2)/2
                    rx = abs(x2-x1)/2; ry = abs(my2-my1)/2
                    page.draw_oval(fitz.Rect(cx-rx, cy-ry, cx+rx, cy+ry),
                                   color=draw_color, width=width, overlay=True)
                elif subtype in ("line", "arrow"):
                    page.draw_line(fitz.Point(x1, my1), fitz.Point(x2, my2),
                                   color=draw_color, width=width, overlay=True)
                    if subtype == "arrow":
                        import math
                        angle = math.atan2(my2-my1, x2-x1)
                        hs = max(8, width*4)
                        tip = fitz.Point(x2, my2)
                        p1 = fitz.Point(x2 - hs*math.cos(angle-0.4), my2 - hs*math.sin(angle-0.4))
                        p2 = fitz.Point(x2 - hs*math.cos(angle+0.4), my2 - hs*math.sin(angle+0.4))
                        page.draw_polyline([p1, tip, p2], color=draw_color, fill=draw_color, width=1, overlay=True)

            elif edit_type == "signature":
                img_data = edit.get("imageData", "")
                if img_data and img_data.startswith("data:"):
                    img_data = img_data.split(",", 1)[1]
                try:
                    img_bytes = base64.b64decode(img_data)
                    sig_rect = fitz.Rect(x, page_height - y - h, x + w, page_height - y)
                    page.insert_image(sig_rect, stream=img_bytes, overlay=True)
                except Exception as e:
                    print(f"Signature insert error: {e}")
